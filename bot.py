...
def Country_find(your_country):
    for country in data['countries']:
        if your_country in country["title"]:  # вместо России надо запросить их страну
            return country


def Region_find(your_country, your_region):
    for region in Country_find(your_country)["regions"]:
        if your_region in region["title"]:  # сюда их регион надо запросить
            return region
...
def make_url(your_country, your_region, your_name_from, your_name_to):
    url = f"https://api.rasp.yandex.net/v3.0/search/?apikey={API_KEY}&from={station_code_find(your_country, your_region, your_name_from)}&to={station_code_find(your_country, your_region, your_name_to)}&date={make_your_date()}&limit=200"  # обработать дату норм
    return url
